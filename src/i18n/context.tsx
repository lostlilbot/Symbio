...
async function loadTranslations(lang: Language): Promise<Translations> {
  if (lang === "en") {
    ...
    enCache = (mod.default || mod) as Translations;
    return enCache;
  }
  if (esCache) return esCache;
  const mod = await import("./translations/es.json");
  esCache = (mod.default || mod) as Translations;
  return esCache;
}
...
export function I18nProvider({ children, defaultLanguage = "en" }: { children: ReactNode; defaultLanguage?: Language }) {
  ...
  useEffect(() => {
    let cancelled = false;
    async function init() {
      const data = await loadTranslations(defaultLanguage);
      if (!cancelled) {
        setTranslationsMap((prev) => ({ ...prev, [defaultLanguage]: data }));
        setReady(true);
      }
    }
    init();
  ...
  return <I18nContext.Provider value={{ language, setLanguage, t, ready }}>{children}</I18nContext.Provider>;
}
