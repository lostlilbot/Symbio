            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                <span className="px-4 py-2 rounded-full bg-cyan-500/10 text-cyan-300 text-xs font-semibold border border-cyan-500/20">
                  {t("compliance.badges.zeroDataTraining")}
                </span>
                <span className="px-4 py-2 rounded-full bg-emerald-500/10 text-emerald-300 text-xs font-semibold border border-emerald-500/20">
                  {t("compliance.badges.ferpa")}
                </span>
                <span className="px-4 py-2 rounded-full bg-violet-500/10 text-violet-300 text-xs font-semibold border border-violet-500/20">
                  {t("compliance.badges.deidentified")}
                </span>
              </div>
            </div>
            <div className="grid grid-cols-1 gap-4">
              {[
                { titleKey: "compliance.items.zeroDataTraining.title", descKey: "compliance.items.zeroDataTraining.desc" },
                { titleKey: "compliance.items.ferpa.title", descKey: "compliance.items.ferpa.desc" },
                { titleKey: "compliance.items.deidentified.title", descKey: "compliance.items.deidentified.desc" },
              ].map((item) => (
                <div key={item.titleKey} className="rounded-2xl border border-white/10 bg-white/5 p-5">
                  <h4 className="font-semibold text-white mb-1">{t(item.titleKey)}</h4>
                  <p className="text-xs text-slate-400 leading-relaxed">{t(item.descKey)}</p>
                </div>
              ))}
            </div>
          </div>
