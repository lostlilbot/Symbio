...
const tracks = [
  { value: "ai-engineering", labelKey: "admissions.tracks.aiEngineering" },
  { value: "human-leadership", labelKey: "admissions.tracks.humanLeadership" },
  { value: "ethics-society", labelKey: "admissions.tracks.ethicsSociety" },
  { value: "interdisciplinary", labelKey: "admissions.tracks.interdisciplinary" },
];
...
export default function EnrollmentForm() {
  ...
  const validateStep1 = (): boolean => {
    ...
    if (!formData.fullName.trim()) newErrors.fullName = t("admissions.errors.fullNameRequired");
    if (!formData.age.trim()) newErrors.age = t("admissions.errors.ageRequired");
    if (!formData.grade) newErrors.grade = t("admissions.errors.gradeRequired");
    if (!formData.email.trim() || !formData.email.includes("@")) newErrors.email = t("admissions.errors.emailRequired");
    if (!formData.country.trim()) newErrors.country = t("admissions.errors.countryRequired");
    setErrors(newErrors);
  ...
  const validateStep2 = (): boolean => {
    const newErrors: Record<string, string> = {};
    if (!formData.track) newErrors.track = t("admissions.errors.trackRequired");
    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };
  ...
  const validateStep3 = (): boolean => {
    const newErrors: Record<string, string> = {};
    if (!formData.statement.trim() || formData.statement.trim().length < 30) newErrors.statement = t("admissions.errors.statementRequired");
    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };
    ...
    <section id="admissions" className="py-16 relative">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-slate-400">{t("admissions.subtitle")}</p>
        </div>
        ...
        <div className="glass rounded-3xl p-6 sm:p-10">
          ...
          {step === 1 && (
            <div className="space-y-5 animate-fade-in-up">
              <h3 className="text-xl font-semibold mb-2">{t("admissions.step1Title")}</h3>
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">{t("admissions.fields.fullName")}</label>
                <input type="text" value={formData.fullName} onChange={(e) => update("fullName", e.target.value)} placeholder={t("admissions.placeholders.fullName")} className={inputClass} />
                {errors.fullName && <p className="text-red-400 text-xs mt-2">{errors.fullName}</p>}
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-2">{t("admissions.fields.age")}</label>
                  <input type="number" value={formData.age} onChange={(e) => update("age", e.target.value)} placeholder={t("admissions.placeholders.age")} className={inputClass} />
                  {errors.age && <p className="text-red-400 text-xs mt-2">{errors.age}</p>}
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-2">{t("admissions.fields.grade")}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">{t("admissions.fields.email")}</label>
                <input type="email" value={formData.email} onChange={(e) => update("email", e.target.value)} placeholder={t("admissions.placeholders.email")} className={inputClass} />
                {errors.email && <p className="text-red-400 text-xs mt-2">{errors.email}</p>}
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">{t("admissions.fields.country")}</label>
                <input type="text" value={formData.country} onChange={(e) => update("country", e.target.value)} placeholder={t("admissions.placeholders.country")} className={inputClass} />
          ...
          {step === 2 && (
            <div className="space-y-5 animate-fade-in-up">
              <h3 className="text-xl font-semibold mb-2">{t("admissions.step2Title")}</h3>
              <p className="text-sm text-slate-400 mb-4">{t("admissions.fields.track")}</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {tracks.map((track) => (
                  ...
                  >
                    <span className="font-semibold">{t(track.labelKey)}</span>
                  </button>
                ))}
              </div>
          ...
          {step === 3 && (
            <div className="space-y-5 animate-fade-in-up">
              <h3 className="text-xl font-semibold mb-2">{t("admissions.step3Title")}</h3>
              <p className="text-sm text-slate-400 mb-4">{t("admissions.fields.statement")}</p>
              <textarea
                value={formData.statement}
                onChange={(e) => update("statement", e.target.value)}
                rows={6}
                placeholder={t("admissions.placeholders.statement")}
          ...
          {step === 4 && (
            <div className="text-center space-y-6 animate-fade-in-up">
              ...
              <h3 className="text-2xl font-bold">{t("admissions.step4Title")}</h3>
              <p className="text-slate-400">{t("admissions.summary.applicationReceived")}</p>
              <div className="glass rounded-xl p-5 text-left space-y-2">
                <p className="text-sm text-slate-400">{t("admissions.summary.referenceId")}</p>
                ...
                <div className="pt-3 border-t border-white/10 text-xs text-slate-400 space-y-1">
                  <p><span className="font-semibold text-slate-300">{t("admissions.summary.name")}:</span> {formData.fullName}</p>
                  <p><span className="font-semibold text-slate-300">{t("admissions.summary.grade")}:</span> {formData.grade}</p>
                  <p><span className="font-semibold text-slate-300">{t("admissions.summary.track")}:</span> {tracks.find(t => t.value === formData.track)?.labelKey || formData.track}</p>
                </div>
              </div>
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                ...
                >
                  {t("admissions.actions.sendWhatsApp")}
                ...
                >
                  {t("admissions.actions.sendInquiry")}
                </button>
              </div>
            </div>
          )}
          ...
          {step < 4 && (
            <div className="mt-8 flex justify-end">
              ...
              >
                {step === 3 ? t("admissions.actions.submit") : t("admissions.actions.continue")}
